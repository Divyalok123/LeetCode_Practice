...
#define ss second;
class Solution {
    int isfine(int i, int j, int n, int m) {
    ...
    int orangesRotting(vector<vector<int>>& grid) {
        ...
        while (!q.empty()) {
            ...
            while (s--) {
                ...
                if (isfine(i-1, j, n, m) && grid[i-1][j] == 1) {
                    grid[i-1][j] = 2; ones--; q.push({ i-1, j });
                }
                if (isfine(i, j-1, n, m) && grid[i][j-1] == 1) {
                    grid[i][j-1] = 2; ones--; q.push({ i, j-1 });
                }
                if (isfine(i+1, j, n, m) && grid[i+1][j] == 1) {
                    grid[i+1][j] = 2; ones--; q.push({ i+1, j });
                }
